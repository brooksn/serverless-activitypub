...
describe('outbox', () => {
  ...
  describe('outboxPOST', () => {
    ...
    it('should accept an object and return its id in the Location header', done => {
      const postString = JSON.stringify(createNote)
      const event = {body: postString, pathParameters: {username: process.env.TEST_USERNAME}, headers: {Authorization: `bearer ${goodClientToken}`}}
      outboxPOST(event, {}, (err, res) => {
    ...
    it('should accept a valid Create activity and return its id in the Location header', done => {
    ...
    it('should reject an object without an Authentication header', done => {
      const postString = JSON.stringify(objectJson)
      outboxPOST({body: postString, pathParameters: {username: process.env.TEST_USERNAME}, headers: {}}, {}, (err, res) => {
        const body = JSON.parse(res.body)
        assert.equal(500, res.statusCode)
        assert.equal('A jwt was not found. ', body.message)
        done()
      })
    })
    
    it('should reject an object without an unauthorized bearer token in the Authorization header', done => {
      const postString = JSON.stringify(objectJson)
      outboxPOST({body: postString, pathParameters: {username: process.env.TEST_USERNAME}, headers: {Authorization: `bearer ${badClientToken}`}}, {}, (err, res) => {
        const body = JSON.parse(res.body)
        assert.equal(500, res.statusCode)
        assert.equal('The token does not include "post" in its scope.', body.message)
        done()
      })
    })
    
    it('should accept a valid Follow activity and return its id in the Location header', done => {
      const postString = JSON.stringify(objectJson)
      const event = {body: postString, pathParameters: {username: process.env.TEST_USERNAME}, headers: {Authorization: `bearer ${goodClientToken}`}}
      outboxPOST(event, {}, (err, res) => {
        assert.equal(201, res.statusCode)
    ...
    it('should accept a valid Update activity and return its id in the Location header', done => {
      const postString = JSON.stringify(objectJson)
      const event = {body: postString, pathParameters: {username: process.env.TEST_USERNAME}, headers: {Authorization: `bearer ${goodClientToken}`}}
      outboxPOST(event, {}, (err, res) => {
        assert.equal(201, res.statusCode)
    ...
    it('should accept a valid Like activity and return its id in the Location header')
    
    it('should accept a valid Delete activity and return a Tombstone object id in the Location header')
    
    it('should accept a valid Add activity and return its id in the Location header')
    
    it('should accept a valid Remove activity and return its id in the Location header')
    
    it('should accept a valid Block activity and return its id in the Location header')
    
    it('should accept a valid Undo activity and return its id in the Location header')
